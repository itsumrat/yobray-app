...
class _EditJobPageState extends State<EditJobPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(titleSpacing: 0, title: Text("Update Job")),
      body: Form(
        key: _formKey,
        child: ListView(
  ...
  Future<void> _submit(String status) async {
    ...
    job.jobTitle = _jobTitleController.text;
    job.description = _descriptionController.text;
    job.budget = _priceController.text;
    job.startingTime = startDate;
    job.endingTime = endDate;
    job.status = status;
    // job.createdAt = null;
    // job.updatedAt = null;
    ...
    myShowDialog();
    final isOk = await controller.updateJob(job, job.status.toString());
